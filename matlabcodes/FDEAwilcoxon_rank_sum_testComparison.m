function s = FDEAwilcoxon_rank_sum_testComparison()
    
    %x1=[1 2 3 4 5]
    %x2=[10 ]
    thebaseDir='D:\FDEA2016\Codes\abcgenerations\recompileWFG-DTLZ\FDEA\FDEAdtlzcomparison\';    
    performancetype='Ss';
    writedir=strcat(thebaseDir,'\',performancetype,'dtlztest.txt');
    writerank=strcat(thebaseDir,'\',performancetype,'dtlzranktest.txt');
    writeobjectivescore=strcat(thebaseDir,'\',performancetype,'dtlzobjectivescoretest.txt');
   ...
   % algorithms=[{'fdea'},{'fdea'},{'fdea'},{'fdea'}];
    ...
    for problemNo=problemNos
        ...
        for dimension=[7 10 15];
            ...
            withoutref=make_array_fix_length(getperformanceData(strcat(thebaseDir,'\withoutcluster\2','\','fdea',problem,num2str(problemNo),num2str(dimension),'.txt'),performancetype),seed)
            pareto=make_array_fix_length(getperformanceData(strcat(thebaseDir,'\pareto\2','\','fdea',problem,num2str(problemNo),num2str(dimension),'.txt'),performancetype),seed)
            gaussian=make_array_fix_length(getperformanceData(strcat(thebaseDir,'\gaussian\2\hype','\','fdea',problem,num2str(problemNo),num2str(dimension),'.txt'),performancetype),seed)
            sigmoid=make_array_fix_lengthFDEA(getperformanceData(strcat(thebaseDir,'\sigmoid\2','\','fdea',problem,num2str(problemNo),num2str(dimension),'.txt'),performancetype),seed)
