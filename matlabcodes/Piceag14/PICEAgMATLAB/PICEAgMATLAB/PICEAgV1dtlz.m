...
for gen = 1:maxGen
    ...
    % You can determine the Upper bound either by non-dominated solutions only,
    % or by the entire merged solutions jointF.
    % NB: floor(jointF.*100)/100 is to ingore the small difference such as 
    % 1.11113 and 1.111,
    nonDomjointFix = find_nd(floor(jointF.*100)/100);
    nonjointF =  jointF(logical(nonDomjointFix),:);
    goalUpper = alpha*max(nonjointF,[],1);
    goalLower = min([jointF;goalLower],[],1);
