function f=sidFinalRunWithParam()




seedNo=[0.1 0.3 0.5 .9];

%seedNo=[0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.95]
%seedNo=[0.05,0.15,0.2,0.25,0.3,0.35,0.4,0.45]
...
%genDirectory='gen\';
genDirectory='C:\Users\secret\Desktop\sidCleanJmetal\jmetal\abcGenerations\50\';
paramGenDirectory=strcat(genDirectory,num2str(paramValue),'\');
...
paramGen=strcat(genDirectory,algo_name(1,:),problem_name(i1,:), num2str(dimension(i2)),'.txt');

paramGen

fid = fopen(paramGen,'a+');

fprintf(fid,'\nInward Parameter value = %d\t\t\n\n',paramValue);
fprintf(fid,'\nPopulation = 150\t\t\n\n');
...
theSeed=num2str(theSeedNum);
pfDirectory=strcat(paramGenDirectory,theSeed,'\');
