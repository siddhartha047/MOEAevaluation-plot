...
dirName='D:\FDEA2016\Codes\abcgenerations\recompileWFG-DTLZ\FDEA\specialProblem\paretoBox\2\';
...
filename = strcat(filedirectory,'PiceagRectangleIII', num2str(testNo), '_',num2str(objvNo),'.pf')
varfilename = strcat(filedirectory,'PiceagRectangleIII', num2str(testNo), '_',num2str(objvNo),'.pfvar')
...
bounds=[zeros(1, nVar)-1000000000000; ones(1,nVar)*1000000000000];  % bounds of decision varialbes
...
for gen = 1:maxGen
   %% obtain new solutions by crossover and mutation
    ...
    if gen == maxGen
        ...
        fid = fopen(varfilename,'w');
        
        dlmwrite(filename,CurrentPF,'-append','delimiter','\t','newline','pc');
        dlmwrite(varfilename,CurrentPS,'-append','delimiter','\t','newline','pc');
        fclose(fid);       
    end
%end
%end
